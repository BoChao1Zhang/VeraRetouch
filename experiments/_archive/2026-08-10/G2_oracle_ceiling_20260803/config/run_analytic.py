...
def run(track: str, index_path: Path | None, out_dir: Path, limit: int | None,
        ...
        estimator: str = "affine", also_const: bool = True) -> dict:
    ...
    with open(out_dir / f"per_image_{track}.jsonl", "w", encoding="utf-8") as f:
        for p in per:
            f.write(json.dumps(p, ensure_ascii=False) + "\n")

    agg = aggregate(track, per)
    with open(out_dir / f"agg_{track}.json", "w", encoding="utf-8") as f:
