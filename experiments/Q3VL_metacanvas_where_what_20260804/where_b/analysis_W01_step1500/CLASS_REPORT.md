...
**长尾集合**（统计口径）= 主榜 context 下 softIoU < 0.3 的全部 local 样本，共 **89** 个（占 local 的 22.2%）；其中最差 12 个 + 每个维度最差类别的最差样本另出联图。机制标签是多标签；`primary` 按「越上游越优先」的固定顺序取一个。

| 失败机制 | primary 计数 | primary 占比 | 命中计数（多标签） | 命中占比 | 未测样本数 |
| --- | ---: | ---: | ---: | ---: | ---: |
| 面积失配（pred_mean vs gt_mean） | 62 | 69.7% | 87 | 97.8% | 0 |
| s 轴塌缩（std(s)/std(s*) 过小） | 12 | 13.5% | 12 | 13.5% | 0 |
| oracle 天花板低（basis 表达上界） | 11 | 12.4% | 11 | 12.4% | 0 |
| 未归因 | 2 | 2.2% | 2 | 2.2% | 0 |
| s 场方向错（w_dir 与 oracle 夹角大） | 1 | 1.1% | 3 | 3.4% | 77 |
| s 错（换 oracle s 就修好） | 1 | 1.1% | 1 | 1.1% | 77 |
| 打不过零参数中心先验 | 0 | 0.0% | 29 | 32.6% | 0 |
...
**瓶颈结论**：长尾里 primary 占比最高的机制是 **面积失配（pred_mean vs gt_mean）**（69.7%）；未归因样本占 2.2%。
