...
## S5.5 · A5-B1 指令条件性框架 —— **已完成，开放项已关闭**

三条负控制 + **两类**配对控制全部实现并接线，`evaluate_arm` 出七块上下文板
（`gt` / `generated` / `null` / `shuffled` / `irrelevant_words` / `fixed_phrase` / `antonym`）。

主 agent 裁定（2026-08-05）：**两类控制都进评测**，方向相反：
...
详细数据见 NOTES §12.1；antonym 控制以文本变换补上了这一侧，且**不需要新数据产物**。
