...
**长尾集合（主榜口径）** = 主榜 context 下 softIoU < 0.3 的全部 local 样本，共 **81** 个（占 local 的 20.2%）。**辅助口径** = 最差 10%（n=40，截断在 softIoU 0.202）。
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 面积失配（pred_mean vs gt_mean） | 54 | 66.7% | 78 | 96.3% | 57.5% | 0 |
| s 轴塌缩（std(s)/std(s*) 过小） | 11 | 13.6% | 13 | 16.0% | 10.0% | 0 |
| oracle 天花板低（basis 表达上界） | 10 | 12.3% | 10 | 12.3% | 25.0% | 0 |
| 未归因 | 3 | 3.7% | 3 | 3.7% | 0.0% | 0 |
| s 场方向错（w_dir 与 oracle 夹角大） | 2 | 2.5% | 6 | 7.4% | 5.0% | 69 |
| s 错（换 oracle s 就修好） | 1 | 1.2% | 4 | 4.9% | 2.5% | 69 |
| rho 错（换 oracle rho 就修好） | 0 | 0.0% | 1 | 1.2% | 0.0% | 69 |
| 打不过零参数中心先验 | 0 | 0.0% | 23 | 28.4% | 0.0% | 0 |
...
**瓶颈结论**：长尾里 primary 占比最高的机制是 **面积失配（pred_mean vs gt_mean）**（主榜口径 66.7%，辅助口径 57.5%）；未归因样本占 3.7%。
