...
## 十四、soft-IoU 形式的显式契约（Where-A 结果审阅 B-1 的连带口径问题）

日期：2026-08-06 ｜ 触发：Where-A 结果审阅 B-1（公用口径，影响 Where-B 判据表）

### 14.1 问题

soft-IoU 的**积形式** `sum(p·g) / sum(p+g−p·g)` 实测（B-1）：

| 量 | 值 |
|---|---:|
| 与 **GT mask 软度**的相关 | **0.955** |
| 与**拟合质量**的相关 | **−0.003** |
| **完美预测**的天花板中位 | **0.786** |

若 §5.6 的 `soft-IoU >= 0.75` 用积形式读，它排的是「GT 有多软」而不是「场对不对」，
**与 AUC 被禁是同一个失败机制**——而且天花板 0.786 意味着一个**完美**的场也几乎卡在门线上。

### 14.2 本仓库现状核实（先查再改）

`q3vl/whereb/metrics.py` 的 `soft_iou_value` **本来就是 min/max**。实测：

| 用例 | metrics 实测 | min/max | 积形式 |
|---|---:|---:|---:|
| 软 GT 的**完美预测**（n=2000 随机软场） | **1.0000** | 1.0000 | **0.5054** |
| p=g=0.5 | 1.0000 | 1.0000 | 0.3333 |
| p=g=0.25 | 1.0000 | 1.0000 | 0.1429 |
| 随机软场对 | 0.5154 | 0.5154 | 0.3336 |

**无需改算法**。本次做的是把它从「默认参数恰好是对的」升级成**显式契约**：
`soft_iou_value` 现在**读 `config.SOFT_IOU_KIND`**，而不是依赖 `losses.soft_iou` 的默认值
——默认值是一次编辑就能翻掉的东西。

> 排查过程里的一个自伤教训：我第一版探针用 `abs(v-mm)<1e-9` 判形式，
> 结果把 `float32` 参考值与 `double` 实算值之间约 **6e-8** 的舍入差判成了「PRODUCT」。
> **判别阈值必须比被判别的两个量之间的真实差距小、比数值噪声大**；
> 这里两种形式差 0.5 量级，而我用了 1e-9。改成「离哪个更近」后结论立刻正确。

### 14.3 落地

1. **代码**：`soft_iou_value` 显式读 `SOFT_IOU_KIND`；`config.SOFT_IOU_KIND = "minmax"` 附完整依据注释。
2. **负向断言测试** `tests/test_soft_iou_form.py`（**14 条**）：
   - 软 GT 的完美预测**恒为 1.0**（4 个软度档），而积形式 <0.6；
   - **对 GT 软度不敏感**：min/max 在 5 个软度档上极差 <1e-4，积形式极差 >0.5
     （直接否掉 B-1 的 0.955 相关）；
   - `soft_iou_value` 源码里必须出现 `SOFT_IOU_KIND`（不许退回默认参数）；
   - criteria 路径（`metrics` / `evaluate`）的 **AST 扫描**中不得出现 `"prod"` 字符串常量
     ——用 AST 而非文本扫描，因为这条路径上的 docstring **故意点名**该形式来解释为何禁用，
     grep 会把解释本身判成违规。
3. **协议**：§5.6 判据表加**脚注 1**（min/max 定档 + B-1 的三个相关性数字）、
   **脚注 2**（「相对 oracle」的分母）；§17.2 增一节记录本次口径明文化。

### 14.4 「相对 oracle ≥85%」的分母（脚注 2）

> **⚠ 本节已被 amendment A-6 更正（2026-08-10，依据 REVIEW-result-W1 §2.1b / P1）。**
> 下面横线以上是**更正后**的口径；横线以下保留原文，仅作留档，**不得再引用**。
>
> **实际分母 = 逐样本、交付档（hi）的 oracle 天花板**：
> `ratio = median_i( soft_iou_i / oracle_soft_iou_i )`，
> 由 `sample_metrics` 对 `mask_hi` 计算，W1 实测中位 **0.9651（W01）/ 0.9759（W02）**。
> `ORACLE_CEILING_LOW_MINMAX`（0.827 / 0.835）是 **low 档参考常量**，判据路径从未读它。
>
> **本交付已产出数字的口径**：`57.94%（W01）/ 58.19%（W02）` = **逐图 hi 档比值的中位数**。
> 若改按 low 档常量分母读，同一批数据是 67.29% / 67.27%（两者都不过 85% 的门，结论不变）。
>
> **原文的「绝对值那行更紧」推论在现行分母下是反的**：0.75 只相当于 **77.7% / 76.9%**，
> **低于 85%**，即**相对那行才是更紧的那个**。
>
> ---
>
...
分母 = Where-A 逐图 oracle 天花板，**low 档、min/max 形式**：
`band 0.827` / `cband12 0.835`（钉在 `config.ORACLE_CEILING_LOW_MINMAX`）。
换档位或换形式都会**静默改变这条 gate**。

读表时有用的推论（单测里也断言了）：天花板约 0.83 时，
绝对值那行 `soft-IoU >= 0.75` 相当于 **90.7%**（band）/ **89.8%**（cband12）的 oracle 占比，
**高于本行的 85%** —— 两行同时生效时**绝对值那行才是更紧的**。
