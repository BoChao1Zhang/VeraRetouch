...
**（2026-08-05 重跑：接入 amendment A-2/A-3 之后，且按审阅 N-1 改在战役环境 `/home/bc/envs/q3vl_sft/bin/python`
（3.12.12 / torch 2.10.0+cu128）出数）**

| 逐轮均值 | T01 (WC-0+FG48) | T08 (WC-3+SB48) |
|---|---|---|
| total loss | 1.2501 → 1.2118 → 1.1786 → 1.1631 → 1.1651 → **1.1446** | 1.2473 → 1.1865 → 1.1413 → 1.1199 → 1.1213 → **1.1121** |
| `L_func` | 0.20177 → **0.19044** | 0.20034 → **0.17999** |
| `L_hc` | 0.16326 → **0.13928** | 0.15991 → **0.11743** |
| `L_style_cos` | 0.83193 → **0.59761** | 0.83376 → **0.59402** |
| `L_style_dist`（A-2 新口径） | 0.51772 → **0.08432** | 0.51800 → **0.31210** |
| `L_bake` | 0.00102 → 0.00106（≈ Charbonnier ε，已在数值地板上） | 0.00105 → 0.00107 |
