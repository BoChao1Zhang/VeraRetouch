...
class LuaConverter:
    ...
    def _escape_lua_string(s):
        """Escape a Python string so it is a valid Lua double-quoted string literal.

        Lua's `"..."` strings cannot span raw newlines and must escape `\\` and `"`.
        Model output sometimes leaks multi-line reasoning text (with quotes/newlines)
        into a settings value; without escaping, the generated config.lua fails to
        load with errors like `unfinished string near '"..."'`. Non-ASCII bytes
        (e.g. UTF-8 Chinese) are valid inside Lua strings and are left untouched.
        """
        out = []
        for ch in s:
            o = ord(ch)
            if ch == '\\':
                ...
                # Other ASCII control characters -> Lua decimal escape.
                out.append('\\%d' % o)
            else:
                out.append(ch)
        return ''.join(out)
    ...
    def to_lua(obj, indent='    ', level=0):
        ...
        if isinstance(obj, bool):
            ...
            result = '"%s"' % LuaConverter._escape_lua_string(obj)
