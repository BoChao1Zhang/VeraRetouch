...
Restored 2026-08-14 from commit ``bdf93d3`` (it was archived out of the tree by
``484f22e``) into ``q3vl/whereb/scripts/`` so the EPR-018..023 wave can drive
the v2seg train-split regeneration as 8 shards across two cards; the only edits
are the repo-root depth and this note.

Usage (one shard per queue job; ``waves/epr018_023_genwhere.sh`` is the driver):
    python -m q3vl.whereb.scripts.genctx_shard --split train --shard 0 \
        --num-shards 8 \
        --out-root   /home/bc/data/runs/where_b/genwhere_v2seg/_shards/train \
        --report-dir /home/bc/data/runs/where_b/genwhere_v2seg/_reports/train \
        --checkpoint /home/bc/data/runs/q3vl_base_sft_v2seg_20260814/checkpoint-4976 \
        --batch-size 64
then merge the 8 published roots into the one the arms read (``--genctx-dir``
of ``run_amort_arm`` points at the PARENT of that directory):
    python -m q3vl.whereb.scripts.merge_genctx --split train \
        --mode two_segment --num-shards 8 \
        --shard-root /home/bc/data/runs/where_b/genwhere_v2seg/_shards/train \
        --out        /home/bc/data/runs/where_b/genwhere_v2seg/train \
        --report     /home/bc/data/runs/where_b/genwhere_v2seg/_reports/genctx_train.json
"""
...
# The file lives at <repo>/q3vl/whereb/scripts/genctx_shard.py, so the repo root
# is parents[3]  (scripts -> whereb -> q3vl -> repo).  It was parents[4] while
# the file lived under experiments/<campaign>/where_b/genctx_jobs/.
