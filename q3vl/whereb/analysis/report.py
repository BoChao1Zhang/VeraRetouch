...
def render_report(payload: Mapping[str, Any]) -> str:
    ...
    A("## 4. 长尾归因")
    A("")
    A(f"**长尾集合**（统计口径）= 主榜 context 下 softIoU < {meta.get('tail_cut')} 的全部 local 样本，"
      f"共 **{summ['n_tail']}** 个（占 local 的 {100*summ['n_tail']/max(1, overall.get('n_local') or 1):.1f}%）；"
      f"其中最差 {meta.get('n_viz_samples')} 个 + 每个维度最差类别的最差样本另出联图。"
    ...
    A(f"**瓶颈结论**：长尾里 primary 占比最高的机制是 **{_MECH_ZH.get(summ['bottleneck'], summ['bottleneck'])}"
      f"**（{100*summ['bottleneck_primary_share']:.1f}%）；未归因样本占 {100*summ['unexplained_share']:.1f}%。")
    A("")
