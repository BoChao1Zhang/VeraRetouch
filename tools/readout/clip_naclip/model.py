...
class VisionTransformer(nn.Module):
    ...
    # clearclip: VeraRetouch add-on (see VENDOR.md) -- softmax(qq^T*scale), arch must be 'reduced'
    def set_params(self, arch, attn_strategy, gaussian_std):
        assert arch in ['reduced', 'vanilla']
        assert attn_strategy in ['naclip', 'nonly', 'kk', 'csa', 'vanilla', 'clearclip']
        assert attn_strategy != 'csa' or arch == 'vanilla'
        assert attn_strategy != 'clearclip' or arch == 'reduced'
    ...
    def forward(self, x: torch.Tensor, return_all=False, extra_tokens: int = 0):
        B, nc, w, h = x.shape
        ...
        # --- VeraRetouch add-on: test-time registers (Jiang et al., arXiv 2506.08010) ---
        # extra zero-embedding tokens appended AFTER the positional embedding, exactly as
        # test-time-registers@860df43 clip/clip/transformer.py:805-822.
        if extra_tokens > 0:
            x = torch.cat(
                [x, torch.zeros(x.shape[0], extra_tokens, x.shape[-1],
                                dtype=x.dtype, device=x.device)], dim=1)
        self._n_extra_tokens = extra_tokens
        ...
        for blk in self.transformer.resblocks[:-1]:
            x = blk(x)
        # VeraRetouch add-on: 末层之前的 hidden（LND→NLD），三个算子完全相同，
        # 因此高范数 outlier（D-0）是骨干与图像的属性，与 attention 手术无关。
        self._prelast_hidden = x.permute(1, 0, 2)
        ...
        self._last_hidden = x    # VeraRetouch add-on: pre-ln_post hidden, for D-0 norm diagnostics
    ...
    def custom_attn(self, attn_layer, x, n_patches, return_attn=False, with_attn=False):
        ...
        n_extra = getattr(self, '_n_extra_tokens', 0)
        if self.attn_strategy in ['naclip', 'nonly']:
            cache_key = (n_patches, n_extra)
            addition = self.addition_cache.get(cache_key)
            if addition is None:
                window_size = [side * 2 - 1 for side in n_patches]
                window = VisionTransformer.gaussian_window(*window_size, std=self.gaussian_std)
                addition = VisionTransformer.get_attention_addition(*n_patches, window)
                if n_extra > 0:
                    # VeraRetouch add-on: pad the neighbourhood bias with zero rows/cols for the
                    # appended test-time register tokens (they have no spatial position).
                    m = addition.shape[0]
                    padded = torch.zeros((m + n_extra, m + n_extra), dtype=addition.dtype)
                    padded[:m, :m] = addition
                    addition = padded
                addition = addition.unsqueeze(0).to(x.dtype).to(x.device)
                self.addition_cache[cache_key] = addition
            ...
            # VeraRetouch add-on, verbatim from ClearCLIP@ad68a40
            # open_clip/transformer.py:614-616 (model_type == 'ClearCLIP')
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, return_all=False, extra_tokens: int = 0):
