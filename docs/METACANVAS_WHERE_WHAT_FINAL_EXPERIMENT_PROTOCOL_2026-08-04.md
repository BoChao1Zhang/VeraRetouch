...
> **与红线原文的差距（须主 agent 裁定，已在 NOTES §12.1 出示数据）**：红线写的是
> 「同图两条**相反**指令」。实测 `V_where` 本地 400 条 / 114 个图像组：同图不同指令的样本对
> **712** 对，其中在明暗/冷暖/饱和度任一轴上**语义相反**的有 **147** 对；但**几乎每一对同图样本的
> 目标区域都不同**——「同主体 + 颜色方向相反」只有 19 对，其中 GT mask 真正相同的仅 **2 对**
> （不同主体的 665 对里 mask 相同的只有 1 对；96 个多样本组全部含 ≥2 个不同 mask）。
> 也就是说，**「同图、指令相反、目标区域相同」的样本对在现有语料里不存在**（n=2，统计上不可用）：
> 每条指令都绑定了自己的候选区域，这是数据构造方式决定的。
> 因此本轮落地的是**同图不同指令**的配对差分——它对 Where 阶段恰恰是更贴题的对照
> （Where 的输出由**主体**决定，不由颜色方向决定），但它不是红线字面写的那一个。
> 若要字面对照，可行方案是**反义指令文本变换**（把指令里的 darker↔brighter 等方向词翻转，
> GT mask 保持不变，因此不需要新的数据产物）——但那测的是**不变性**（mask 不应随颜色方向改变），
> 与「配对差分应为正」的方向相反，属于另一类控制。二选一请主 agent 裁定。
