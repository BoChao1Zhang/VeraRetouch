...
>
> **脚注 2 · 「相对 oracle」的分母**（**已由 amendment A-6 重写，2026-08-10**）
> = **逐样本、交付档（hi）** 的 Where-A oracle 天花板，min/max 形式：
> `ratio = median_i( soft_iou_i / oracle_soft_iou_i )`，
> 与 A-WhereA-1 A2 表的 `fits.<readout>.eval.hi.soft_iou_minmax` 同源。
> 换一个档位、换成常量分母、或换成积形式都会**静默改变这条 gate**。
> W1 实测该分母中位 **0.9651（W01）/ 0.9759（W02）**。
>
> `q3vl/whereb/config.py:ORACLE_CEILING_LOW_MINMAX`（`band 0.827` / `cband12 0.835`）
> 是 **low 档参考常量，不是本行的分母**；判据路径从未读过它，
> 只在需要与 low 档并列报告时使用。
>
> 原脚注附带的推论（「天花板约 0.83 ⇒ 绝对值那行更紧」）**已删除**：在现行分母下它是**反的**
> —— 0.75 只相当于 **77.7% / 76.9%**，**低于 85%**，即**相对那行才是更紧的那个**。
>
> **脚注 3 · 分位数一律 nearest-rank**（amendment A-6）。`median` / `p10` / `p90`
> 是最近秩序统计量（`numpy.quantile(method="nearest")` 等价：`sorted[round(p·(n−1))]`），
> **不是**插值分位数。n=400 时 median = 排序后第 201 个元素。
...
| **中心先验基线** | 零参数 `-到画幅中心距离`，**同支撑、同 top-k 规则**。任何「场找到了主体」的主张必须出示**配对 Δ 与 p 值**（**sign-flip 置换检验，10,000 次**；95% CI 另用 2,000 次百分位 bootstrap —— 口径由 **amendment A-6** 校正，原文「bootstrap，2000 次」与实现不符） |
...
## Amendment A-6（2026-08-10，依据 = `where_b/REVIEW-result-W1.md` §2.1a/b/c、P1/P2/P3）

W1 交付后的结果审阅点名了三处**文档与实现口径不一致**。三条都不改任何判据数值，只把
「同一个数字有两种读法」的地方钉成一种。裁定由主 agent 于 2026-08-10 作出（任务卡 EXEC-5 B）。

### A6-1 · 「相对 oracle」的分母改为**逐图 hi 档**（§5.6 脚注 2 重写）

**实现一直是**：`soft_iou_vs_oracle_ratio = median_i( soft_iou_i / oracle_soft_iou_i )`，
其中 `oracle_soft_iou_i` 是**该样本自己的**、**交付档（hi）** 的 Where-A 天花板
（`q3vl/whereb/metrics.py`：`sample_metrics` 收到的 `m_oracle` 与 `mask_hi` 同分辨率）。
W1 实测该分母中位 **0.9651（W01）/ 0.9759（W02）**。

**脚注 2 原文写的是**：分母 = `config.ORACLE_CEILING_LOW_MINMAX`，
**low 档常量** `band 0.827 / cband12 0.835`。

**裁定：以实现为准。** 逐图配对比常量分母更严格（每个样本与自己的天花板比，不被
「容易的样本天花板高」这种构成效应稀释），且 W1 已按此口径产出数据。

| 口径 | W01 | W02 |
|---|---:|---:|
| **现行（逐图 hi 档比值的中位数）** | **57.94%** | **58.19%** |
| 若按旧脚注的常量分母（0.827 / 0.835）读 | 67.29% | 67.27% |

**同时删除脚注 2 附带的那条推论**。原文说「天花板约 0.83 ⇒ 绝对值那行 `>=0.75` 相当于
约 90.7% ⇒ **绝对值那行才是更紧的**」。在现行（逐图 hi）分母下这句话是**反的**：
0.75 只相当于 **77.7% / 76.9%**，**低于 85%**，即**相对那行才是更紧的那个**。
这句话本来是写来防止误判的，现在它本身会造成误判。

`config.ORACLE_CEILING_LOW_MINMAX` 保留为 **low 档参考常量**，**不是 gate 的分母**
（代码里它本来就没有被判据路径读过）；它只在需要与 low 档并列报告时使用。

**已产出数字的口径标注**：W1 的 `57.94% / 58.19%` 一律标注为
「相对**逐图 hi 档** oracle（`fits.<readout>.eval.hi.soft_iou_minmax` 同源）」。
引用旧脚注常量口径的任何数字必须显式写「low 档常量分母」。

### A6-2 · 所有分位数一律 **nearest-rank**（§5.6 脚注 3，新增）

`median` / `p10` / `p90` 在本战役是**最近秩序统计量**（`numpy.quantile(method="nearest")`
的等价实现：`sorted[round(p·(n−1))]`），**不是**插值中位数。n=400 时 median 取排序后第 201 个元素。
实测差异很小（W01 median 0.556502 vs 插值 0.556113，差 3.9e-4），但 `>= 0.75` 这类硬阈值上，
一个口径差就是一次翻案的余地，所以写进协议而不是留给读者猜。

### A6-3 · 配对检验的重采样次数：p 值 **10,000 次 sign-flip 置换**，CI 另有 2,000 次 bootstrap

A-5 原文写「bootstrap，2000 次重采样」，与实现不符，且把两件事混成了一件。实现
（`q3vl/whereb/metrics.py::paired_delta`）是：

| 量 | 方法 | 次数 |
|---|---|---:|
| **p 值** | sign-flip 置换检验（add-one 修正，故恒 > 0） | **10,000** |
| 95% CI | 百分位 bootstrap | 2,000 |

因此所有配对 p 值的下限是 `1/10001 = 9.999e-05`（W1 交付里观测到的正是这个数）。
A-5「中心先验基线」一行的括注据此改为「sign-flip 置换检验，10,000 次；CI 为 2,000 次 bootstrap」。

### 不受影响的部分

判据阈值（`>=0.75` / `>=85%` / `>=0.55` / `>=75%` / …）、gate 行数与顺序、字典序选择规则、
soft-IoU 的 min/max 形式（A-5）、A-WhereA-1 的 A1–A4 全部不变。本 amendment 只动口径叙述。
