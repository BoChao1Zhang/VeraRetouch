...
# 臂 2 变体：教师改本地加载（--teacher_model = 同路径；tuner_type full ⇒ ms-swift 载入一份独立冻结拷贝作固定教师，rlhf_args.py L429-432），
# 教师 logits 为**全词表**（非 API top-k 64）。其余与 train_opsd_full.sh 逐字相同。EPR-052 ENG-2 臂 2。
...
VR_PROBE_OUT="$OUT/probe_segments.jsonl" CUDA_VISIBLE_DEVICES=${GPU:-1} swift rlhf --rlhf_type gkd \
  ...
  --teacher_model "$MODEL" --teacher_model_type qwen3_vl \
  ...
  --external_plugins /workspace/VeraRetouch/veraretouch_sprf/rl/plugins/gkd_probe_v3.py \
