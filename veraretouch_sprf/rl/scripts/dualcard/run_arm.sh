...
(docker compose exec -T -e GPU=1 -e OUT=/data/runs/epr052_rl/arms/$ARM -e STEPS=$STEPS -e SAVE=100000 -e PB=$PB -e GA=$GA \
...
echo "[arm $ARM] finished; probe rows: $(wc -l < "$P" 2>/dev/null || echo 0); early_stop=${stop:-none}"
