#!/usr/bin/env python3
# 源自 experiments/prs/EPR-051_masked-restore-production/stage0/sprf/stage_solver_bk.py @ 2026-09-06（原件未入 git；原 sha256 见 veraretouch_sprf/PROVENANCE.md），逐字复制/仅改导入与路径解析（EPR-052）。
"""EPR-051 · stage0 · sprf · BK-ABL-v2 分支求解器（E1：`stage_solver.py` 一字不改）。

与原件的**唯一**差异：`restore()` 里 `base = model.cond.base(c)` 改为
`model.cond.base(c, edits)` —— ditblk 臂的 K/V token 需要整链编辑描述子，其余臂忽略第二参。
其余（`_call` / 两种求解器 / `rollout_with_metrics` / `ProductionBatch`）逐字相同。
"""
...
def restore(model, batch, path_mode: str, nfe_per_stage: int, depth: int,
    ...
    base = model.cond.base(c, model.edit_descriptor(edits))  # BK 分支：整链编辑进 base
