#!/usr/bin/env python3
# 源自 experiments/prs/EPR-051_masked-restore-production/stage0/sprf/train_sprf.py @ 2026-09-06（原件未入 git；原 sha256 见 veraretouch_sprf/PROVENANCE.md），逐字复制/仅改导入与路径解析（EPR-052）。
...
# --------------------------------------------------------------------------- #
def cond_base(model, feat, amp_dt, scope):
    ...
    if amp_dt is not None and scope == "trunk_only":
        with torch.autocast("cuda", dtype=amp_dt):
            base = model.cond.base(feat)
        return base.float()
    return model.cond.base(feat)
...
def quick_eval(model, samples, blobs, feats, cfg, device, ledger, n_steps,
    ...
    for i, s in enumerate(samples):
        ...
        edits = (None if edit_src is None else
        ...
        for col, ctrl in (("delta_const", torch.zeros_like(feat)),
            ...
            pc, _ = SS.rollout_with_metrics(
                model, model.cond.base(ctrl), yb, ahat, alphas, union, depth, sc,
        ...
        for col, mk in (("delta_edit_null", model.null_edit),
            ...
            pc, _ = SS.rollout_with_metrics(
                model, base, yb, ahat, alphas, union, depth, sc, path_mode,
                n_stages, 1, lut_ids=lut_ids, edits=mk(edits))
...
def main() -> None:
    ...
    frozen = dict(
        ...
        stage_flow=T0.sha256_file(Path(SF.__file__)),
        stage_solver=T0.sha256_file(Path(SS.__file__)),
    ...
    model = SF.SprfModel(in_dim, cfg, n_steps, alpha_mode, depth_values).to(device)
    ...
    #   A12a  第 1 步：film.weight 的**编辑 latent 那几列**梯度非零
    #                  ⇒ 编辑 latent 确实到达了 FiLM 头；
    #   A12b  第 2 步：edit_enc 自身参数梯度非零且无 None
    #                  ⇒ 编码器确实在被训练。
    a12: dict = {}
    ...
    try:
        while step < max_steps and not interrupted:
            for b in dl:
                ...
                base = cond_base(model, feat, amp_dt, scope)
                ...
                loss = book.add("prev", ST.masked_mean(
                ...
                loss.backward()
                if edit_src is not None and len(a12) < 2:
                    enc = model.cond.edit_enc
                    fw = model.cond.film.weight
                    cut = fw.shape[1] - model.cond.edit_latent
                    if "a12a_film_edit_slice" not in a12:
                        g = fw.grad
                        v = 0.0 if g is None else float(g[:, cut:].abs().sum())
                        a12["a12a_film_edit_slice"] = dict(
                            step=step + 1, grad_abs_sum=v,
                            n_edit_columns=int(model.cond.edit_latent))
                        if not v > 0.0:
                            die("A12a FAILED: FiLM 头上「编辑 latent」那几列梯度为 0 "
                                f"({a12['a12a_film_edit_slice']}) —— 编辑 latent "
                                "没有到达 FiLM 头，条件注入「定义了没接线」")
                        ...
                        print(f"[assert] A12 条件注入接线 OK: "
                              f"film 编辑列 |grad| = "
                              f"{a12['a12a_film_edit_slice']['grad_abs_sum']:.3e}; "
                ...
                if step % log_every == 0 or step == 1 or step == max_steps:
                    ...
                    print(f"  step {step}/{max_steps} loss={rec['loss']:.6f} "
                          ...
                          f"peak={rec['gpu_peak_gb']}GB", flush=True)
